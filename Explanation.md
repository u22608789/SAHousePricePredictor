...
### Running the API
Start the backend server to handle predictions:
```bash
uvicorn src.api:app --reload
```
The API will run at `http://localhost:8000`. You can view the auto-generated docs at `http://localhost:8000/docs`.
...
When running `pipeline.py`, you will see the following metrics:

- **RMSE (Root Mean Squared Error)**: Represents the average distance between the predicted price and the actual price. Lower is better. It is in the same units as the target (ZAR).
- **MAE (Mean Absolute Error)**: The average absolute difference between predicted and actual values. It gives a more direct interpretation of "how wrong" the model is on average.
- **R² Score (Coefficient of Determination)**: Indicates how well the independent variables (features) explain the variance in the dependent variable (price).
...
- **Initial Goal**: Scrape data from Property24/Private Property.
- **Pivot**: Due to Terms of Service restrictions on scraping, we switched to using existing Kaggle datasets (`house_prices_train.csv`).
